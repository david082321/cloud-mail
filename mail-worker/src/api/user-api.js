...
app.put('/user/setPwd', async (c) => {
	await userService.setPwd(c, await readJsonBody(c));
	return c.json(result.ok());
});

app.put('/user/setStatus', async (c) => {
	await userService.setStatus(c, await readJsonBody(c));
	return c.json(result.ok());
});

app.put('/user/setType', async (c) => {
	await userService.setType(c, await readJsonBody(c));
	return c.json(result.ok());
});
...
app.post('/user/add', async (c) => {
	await userService.add(c, await readJsonBody(c));
	return c.json(result.ok());
});

app.put('/user/resetSendCount', async (c) => {
	await userService.resetSendCount(c, await readJsonBody(c));
	return c.json(result.ok());
});

app.put('/user/restore', async (c) => {
	await userService.restore(c, await readJsonBody(c));
	return c.json(result.ok());
});
