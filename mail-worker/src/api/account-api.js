...
app.post('/account/add', async (c) => {
	const account = await accountService.add(c, await readJsonBody(c), userContext.getUserId(c));
	return c.json(result.ok(account));
});

app.put('/account/setName', async (c) => {
	await accountService.setName(c, await readJsonBody(c), userContext.getUserId(c));
	return c.json(result.ok());
});

app.put('/account/setAllReceive', async (c) => {
	await accountService.setAllReceive(c, await readJsonBody(c), userContext.getUserId(c));
	return c.json(result.ok());
});

app.put('/account/setAsTop', async (c) => {
	await accountService.setAsTop(c, await readJsonBody(c), userContext.getUserId(c));
	return c.json(result.ok());
});
